...
%   $Revision: 1.19 $  $Date: 2002/03/28 13:26:38 $ 
...
%    for i = 1:8  % number of subplots
     for i = 1:VT.numplots  % number of subplots
    ...
    % xinhui, added  03/07/2004
    %ud.prefs.tilemode = [4 2];
    
    % ud.prefs.tilemode = [VT.numplots/2 2]; % VT.numplots -> 32
    ud.prefs.tilemode = [4 2];
    
    
    %ud.prefs.plots = [1 1 0 0 0 0 0 0]';  
    ud.prefs.plots = [1 1 zeros(1, VT.numplots-2)]';  
...
%%%%% added by xinhui 01/29/2004
    ...
    %%%%% commented (or added) by xinhui 02/02/2004               
    ud.resize.minsize = [ 931.0208  638 ] ;
    ...
    %%%%% commented (or added) by xinhui 02/01/2004         
    ud.resize.leftwidth = sz.bw+36*sz.fus;
...
%%%%% added by xinhui 01/29/2004
    fp = get(VT.handles.vtar, 'position' ) ; 
%     tempSize = get(0, 'screensize') ;
%     if tempSize(3)*0.8 > 1024
%         fp(3) = tempSize(3)*0.8 ;
%     end    
%     if tempSize(4)*0.8 > 768
%         fp(4) = tempSize(4)*0.8 ;
%     end
...
%        'position',[frf(1:2)+sz.fus+[indentPop 0] sz.bw-indentPop sz.uh]+popTweak);
    ...
    % for i=1:8
     for i=1:VT.numplots
        ...
        %%%%% added by xinhui 01/29/2004
%         if i==1 
% %             ud.ht.a(i) = VT.handles.Formant_plot ;
%             set(ud.ht.a(i), ax_props{:}, 'visible','off') ; 
%         elseif i==2
%             ud.ht.a(i) = VT.handles.AreaFunction_plot ;
%             set(ud.ht.a(i), ax_props{:}, 'visible','off') ; 
%         else
%             ud.ht.a(i) = axes(ax_props{:}, 'visible','off');
%         end

        ud.ht.a(i) = axes(ax_props{:}, 'visible','off');
        if i==1 
            VT.handles.Formant_plot = ud.ht.a(i)  ;  % just because we used formant_plot firstly, but removed it 
                                                     % and use a(1)..., due
                                                     % to dislocation of
                                                     % title and labels
        elseif i==2
            VT.handles.AreaFunction_plot = ud.ht.a(i) ;
        end
...
%     ud.titles = {   'Acoustic Response Function' 
...
% 
 ...
 % title('Area Function')
    ...
    % set([th{[1:8]}],{'string'},ud.titles([1:8]))
    %
    % 03/08/2003 added(or commented) by xinhui      
    %
    %in this way, code still works even if length() is less than
    % VT.numplots, which may happen is VT.numplots is too large and I do
    % not want to specify all the titles
    set([th{[1:length(ud.titles)]}],{'string'},ud.titles([1:length(ud.titles)])) ;
...
% 03/08/2003 added(or commented) by xinhui      
    set([th{[1:length(ud.tags)]}],{'tag'},ud.tags)
    set(ud.ht.a(1:length(ud.tags)),{'tag'},ud.tags)
    xh = get(ud.ht.a,'xlabel');
    set([xh{[1:length(ud.tags)]}],{'string'},ud.xlabels,{'tag'},ud.tags)
    yh = get(ud.ht.a,'ylabel');
    set([yh{[1:length(ud.tags)]}],{'string'},ud.ylabels,{'tag'},ud.tags)
...
%     end
    ...
    % 03/08/2004 added (or commented) by xinhui
    uimenu(ud.contextMenu.u, 'label','Acoustic Response','checked', 'on', 'Callback', 'plotcontextmenu(1, 1, 1)') ;
    uimenu(ud.contextMenu.u, 'label','Vocal Tract Area Function', 'checked', 'on','Callback', 'plotcontextmenu(2, 1, 2)') ;
%     uimenu(ud.contextMenu.u, 'label','Acoustic Response','checked', 'on', 'Callback', 'plotcontextmenu(gcbo, 1, 1)') ;
%     uimenu(ud.contextMenu.u, 'label','Vocal Tract Area Function', 'checked', 'on','Callback', 'plotcontextmenu(gcbo, 1, 2)') ;
%     uimenu(ud.contextMenu.u, 'label','Area Function 2', 'checked', 'on','Callback', 'plotcontextmenu(gcbo, 1, 3)') ;
%     uimenu(ud.contextMenu.u, 'label','Area Function 3', 'checked', 'on','Callback', 'plotcontextmenu(gcbo, 1, 4)') ;
%     uimenu(ud.contextMenu.u, 'label','Area Function 4', 'checked', 'on','Callback', 'plotcontextmenu(gcbo, 1, 5)') ;
%     uimenu(ud.contextMenu.u, 'label','Area Function 5', 'checked', 'on','Callback', 'plotcontextmenu(gcbo, 1, 6)') ;
%     uimenu(ud.contextMenu.u, 'label','Area Function 6', 'checked', 'on','Callback', 'plotcontextmenu(gcbo, 1, 7)') ;
%     uimenu(ud.contextMenu.u, 'label','Area Function 7', 'checked', 'on','Callback', 'plotcontextmenu(gcbo, 1, 8)') ;
    set(VT.handles.vtar,'uicontextmenu',ud.contextMenu.u) ;

    % put menu on the menubar, but I do not know how to share the same
    % menus as contextmenu above 
    uimenu(VT.handles.Plot, 'label','Acoustic Response','checked', 'on', 'Callback', 'plotcontextmenu(1, 1, 1)') ;
    uimenu(VT.handles.Plot, 'label','Vocal Tract Area Function', 'checked', 'on','Callback', 'plotcontextmenu(2, 1, 2)') ;

    % 03/07/2004commented by xinhui
...
%     ud.contextMenu.lineprop = uimenu(ud.contextMenu.pickMenu);
    ...
    %%%%% added by xinhui 01/29/2004
    ud.toolbar = VT.toolbar ;  % already created in create_toolbar()
    ...
    %    ud.prefs.plots = [1 1 1 1 1 1 1 1 ] ;
    ud.prefs.plots = [1 1  zeros(1, VT.numplots-2)] ;
...
%             isreal(ud.filt(ud.focusIndex).tf.den));
    ...
    %%%%% added by xinhui 01/29/2004
        ...
        popupCallback = 'filtview(''rulerpopup'')';	
            ...
            %%%%% commented (or added) by xinhui 02/04/2004  
...
%         ruler('init',fig,ud.prefs.plottitles,plotPopVal,popupCallback,...
%             ud.ht.a,[1 2 3 4 5 6 7 8]);
        
        ruler('init',fig,ud.prefs.plottitles,plotPopVal,popupCallback,...
            ud.ht.a,[1:VT.numplots]);
...
%     tbfillblanks(ut,ud.toolbar.lineprop,ud.toolbar)
    ...
    %%%%% commented (or added) by xinhui 02/04/2004    
    saveRulerFlag = 0 ;  % no ruler at the beginning
    ...
    %%%%% commented (or added) by xinhui 02/04/2004    
    eval(get(fig,'resizefcn')) ; % due to saveRulerFlag = 0 ; 
