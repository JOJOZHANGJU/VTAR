 ...
 *        Author: Tarun Pruthi , Zhanyan Zhang, Xinhui Zhou
 *                                      Date: 01/25/2004
 *
 *
...
int calcKZ(double f, double* A, double* l, int nLength, Complex Zterm, property *p, Complex K[2][2], Complex *Z)
{
  ...
  if(k!=nLength)  // break out due to zero at some point
	...
	Ra = l[0] * S/2/pow(A[0],2) * sqrt(omega* p->RHO * p->MU/2) ;
	K[0][0] = 1;
	K[0][1] = Ra + j*La;
	K[1][0] = 0;
	K[1][1] = 1;
	    ...
	    if ( k < (nLength-1) )
	  	{
			...
			Ra1 = l[k+1]* 2*sqrt(A[k+1]*PI) /2/pow(A[k+1],2)*sqrt(omega* p->RHO * p->MU/2) ;  // corrected
	    }
	  	Ca = l[k]*A[k]/p->RHO/pow(p->C0,2)*omega ;
    	Ga = S * (p->ETA-1)/ p->RHO/pow(p->C0,2) * sqrt(p->LAMDA*omega/2/p->CP/p->RHO)*l[k] ;
