...
%     end
    ...
    if strcmp(get(ud.toolbar.markers,'state'),'on')
       ...
       rulerEnable=1;
           ...
           % just to make the plot 's title different , in red
...
%     sptool('setprefs',ud.prefs.rulerPrefsPanelName,p)
    ...
    eval(get(fig,'resizefcn')) ;   % //xinhui resize 
...
%        set(ud.ht.a([1:6]),'xcolor','k','ycolor','k')
%        th=get(ud.ht.a([1:6]),'title');
%        set([th{:}],'color','k') % to get rid of the hightlight plot
...
%         end    
        ...
        % turn off peaks and/or valleys
        if strcmp(get(ud.toolbar.peaks,'state'),'on')
            % peaks are on
            % set(ud.ruler.uimenu.peaks,'checked','off')
            set(ud.toolbar.peaks,'state','off')
            %btnup(fig,'peaksgroup',1)
        ...
        if strcmp(get(ud.toolbar.valleys,'state'),'on')  % valleys are on
            % set(ud.ruler.uimenu.valleys,'checked','off')
            set(ud.toolbar.valleys,'state','off')
            ruler('valleys')
        end    
...
%          ud.toolbar.peaks      ud.toolbar.valleys ],'enable',on_off)
        ...
        % 03/08/2003 added(or commented) by xinhui      
    set([ud.toolbar.track      ud.toolbar.slope ...
         ud.toolbar.peaks      ud.toolbar.valleys ],'enable',on_off)
...
case 'peaks'
    ...
    %%%%% commented (or added) by xinhui 02/03/2004 
    % uiSource seems not used at all , strange !  
...
%          set(ud.toolbar.peaks,'state','off')
         ...
         %if strcmp(get(ud.ruler.uimenu.valleys,'checked'),'on') 
                ...
                %%%%% commented (or added) by xinhui 02/03/2004 
         if strcmp(get(ud.toolbar.valleys,'state'),'on') 
            if  strcmp(ud.ruler.type,'track') | strcmp(ud.ruler.type,'slope')
