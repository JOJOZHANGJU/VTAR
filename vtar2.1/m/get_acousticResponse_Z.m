...
function get_acousticResponse_Z(Category, Type)
% only for sensitiviy function of one single tube, vowel type
...
% constants, can not be modified since they are also used in mex file 
ORAL_MODE = 2 ;
NASAL_MODE = 3 ;
ORALNASAL_MODE = 4 ;
FRICATIVE_MODE = 5 ;
...
if (Category ~= 8)
    for i = 1:1  % for other types of sound instead of vowel, it should be modified, 
...
switch Category
    ...
    case 2  % 'Vowel'
        ...
        [f, AR, Z] = arbitray_model(Fupper, df, deltaL, ORAL_MODE, Tube_transformed, VT.Property) ;
...
% not used here since the geometry got checked elsewhere
% if (~checkFormat(root))
%     return ;
% end
...
% [f, AR, tempZ] = vtar_calc_mex_z(Fupper, df, mode , root, currentTube_Calc, Property) ;

% [f, AR, tempZ] = vtar_calc_mex_z1(Fupper, df, mode , root, currentTube_Calc, Property) ;
%  figure; subplot(211); plot(f,  imag(tempZ) ) ; title('Z in vtar_mex'); hold on ; 
%  subplot(212);  plot(f, 20*log10(abs(AR))) ; 
% 
%  [f, AR, tempZ] = vtar_calc_mex_z1(Fupper, df, mode , root, currentTube_Calc, Property, complex(0,0)) ;
%  figure; subplot(211); plot(f,  imag(tempZ) ) ; title('Z in vtar_mex'); hold on ; 
%  subplot(212);  plot(f, 20*log10(abs(AR))) ; 
...
%  figure; plot(f,  imag(tempZ) ) ; title('Z in vtar_mex'); hold on ; 
% plot(f,  Property.RHO*Property.C0*tan(2*pi*f/35000*17.5)/currentTube_Calc(1).secArea(1), ':' ) ;
% % figure; plot(f,  1./imag(tempZ) ) ; title('1/Z in vtar_mex')
% % figure; plot(f, 20*log10((tempZ))) ; title('1/Z in vtar_mex')
% % figure; plot(f, 20*log10(abs(AR))) ; 
% 
% figure; plot(f,  imag(tempZ) ) ; title('Z in vtar_mex'); hold on ; 
% plot(f,  -Property.RHO*Property.C0*1./tan(2*pi*f/35000*17.5)/currentTube_Calc(1).secArea(1), ':' ) ;
...
% figure; plot(f, 20*log10(abs(tempZ))) ;  title('Z in vtar')
% figure; plot(f, 20*log10(abs(AR))) ; 
% 
