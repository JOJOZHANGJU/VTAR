...
%    tbfillblanks(ut,ud.toolbar.lineprop,ud.toolbar)
   ...
   fvresize(1,fig)
    ...
    %%%%% commented (or added) by xinhui 02/02/2004  
    tempPos = get(VT.handles.frame_areafunction,'position');  % normalized 
    ud.resize.leftwidth = (tempPos(1)+tempPos(3))*fp(3) + ud.sz.ffs ;
...
%              each nonzero entry of plots.
    ...
    % 03/07/2004, xinhui added,  just in my case 
    if(numplots==4)
        numrows = 2;
        numcols = 2;
    elseif (numplots==5)
    ...
    %%%%% commented (or added) by xinhui 02/02/2004      
    % just to make the 1st plot at the bottom by without flip the matrix
    %     pos = flipud(pos);  
    ...
    spacing = [spacing(1:2) spacing(3:4)+3] ;
    ...
    % Added by xinhui to adjust the size of plost , to make the plot more
    % readable , May 18/2004
    
    Max_ratio = 3 ; % 2.5 ;
    Min_ratio = 1.5 ;
    
    for i = 1:length(pos)
        ratio = pos{i}(3) / pos{i}(4) ;
        if (ratio > Max_ratio)
            	% length is much more than height .
           % moving the staring x as  origntal x – 0.5 * (pos(3)- pos{i}(4)*Max_ratio)
            pos{i}(1) = pos{i}(1) + round( 0.5 *(pos{i}(3)-pos{i}(4)*Max_ratio) ) ; % new staring point at x
            pos{i}(3) = round(pos{i}(4)*Max_ratio) ;
            
        elseif (ratio < Min_ratio)
            pos{i}(2) = pos{i}(2) + round( 0.5 *(pos{i}(4)-pos{i}(3)/Min_ratio) ) ; % new staring point at y
            pos{i}(4) = round(pos{i}(3)/Min_ratio) ;
        end
        pos{i} = round(pos{i});
    end
...
% =================================
    ...
    %%%%% commented (or added) by xinhui 02/02/2004  
    % unused so far in this function 
